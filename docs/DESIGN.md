...
Windowed groups are located, moved to `recorded_rect`, and focused before their first event. If the window is missing after `window_timeout` seconds the group is skipped or playback halts, per `on_missing_window`.
...
- The Details panel is rebuilt per event type and syncs to the event on every keystroke.
